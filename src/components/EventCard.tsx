...
const EventCard = ({
  ...
  const getDifficultyColor = (diff: string) => {
    switch (diff) {
      case "E":
        return "bg-difficulty-e text-white";
      case "E+":
        return "bg-difficulty-eplus text-white";
      case "T":
        return "bg-difficulty-t text-white";
      default:
        return "bg-muted";
    }
  };
  return <div className="flex gap-6 py-4 border-b border-border last:border-0 hover:bg-accent/5 transition-colors cursor-pointer -mx-6 px-6" onClick={onClick}>
      ...
      <div className="flex-col gap-0.5 min-w-[140px] flex items-start justify-start">
        <div className="text-sm">{departure}</div>
        {transport !== "none" && <div className="flex items-center gap-1 text-xs text-muted-foreground">
            <span>by</span>
            {TransportIcon && <TransportIcon className="w-3 h-3" />}
            <span>{transportSubtext || (transport === "train" ? "Train" : "Bus")}</span>
          </div>}
        {transport === "none" && transportSubtext && <div className="text-xs text-muted-foreground">{transportSubtext}</div>}
      </div>
      ...
      <div className="flex flex-col items-end gap-1 min-w-[140px]">
        ...
        <div className="flex -space-x-2">
          {participantAvatars.slice(0, 5).map((avatar, idx) => <Avatar key={idx} className="w-6 h-6 border-2 border-background">
